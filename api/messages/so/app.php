...
return [
    ...
    'Hello %name%, Your Demo password reset code is %code%' => 'Salaan %name%, koodkaaga cusub ee furaha sirta ah waa %code%',
    'Hello %name%, Your Demo Activation code is %code%' => 'Salaan %name%, koodkaaga hawlgelinta waa %code%',
    'Hello %name%, Your Demo order for %volume% from %vendor_name% has been rejected' => 'Salaan %name%, dalabaadkaaga Demo ee %volume% ka soo qaaday %vendor_name% waa la diiday',
    'Hello %name%, REF:DEMO-%ref%, You have a %single_shared% Demo order of %volume% to %address%,  %date%. %order_details%' => 'Salaan %name%, TIX:DEMO-%ref%, waxaad haysataa dalabaad %single_shared% Demo oo %volume% ah oo loo diray %address%, %date%. %order_details%',
    'Hello %name%, Your Demo order for %volume% from %vendor_name% has been accepted and ready for delivery on %date%' => 'Salaan %name%, dalabaadkaaga Demo ee %volume% ka soo qaaday %vendor_name% waa la aqbalay oo diyaar u ah soo saarida %date%',
    'Hello %name%, Your Demo order for %volume% from %vendor_name% has been delivered on %date%' => 'Salaan %name%, dalabaadkaaga Demo ee %volume% ka soo qaaday %vendor_name% waa la soo saaray %date%',
    'Hello %name%, Your %single_shared% Demo order for %volume% from %vendor_name% on %date% has been created successfully' => 'Salaan %name%, dalabaadkaaga %single_shared% Demo ee %volume% ka soo qaaday %vendor_name% %date% si guul leh ayaa loo sameeyey',
    'Hello %name%, Your Demo order for %volume% from %vendor_name% on %date% has been rejected' => 'Salaan %name%, dalabaadkaaga Demo ee %volume% ka soo qaaday %vendor_name% %date% waa la diiday',
    'Hello %name%, Your Demo vendor account has been created successfully. Your login credentials are as follows: Phone: %phone% and Password: %password%' => 'Salaan %name%, xisaabtaaga biyoolaha Demo si guul leh ayaa loo sameeyey. Aqoonsigaaga galitaanka waa: Telefoon: %phone% iyo Furaha sirta ah: %password%',
    'Hello %name%, Your Demo vendor account has been reset. Your new login credentials are as follows: Phone: %phone% and Password: %password%' => 'Salaan %name%, xisaabtaaga biyoolaha Demo waa la dib u habeyn. Aqoonsigaaga cusub ee galitaanka waa: Telefoon: %phone% iyo Furaha sirta ah: %password%',
    ...
    'Hello %name%, You have accepted to deliver %volume% to %address% on %date%. REF-%ref%' => 'Salaan %name%, waxaad haysataa dalabaad %volume% ah oo loo diray %address%, %date%. TIX:DEMO-%ref%',
    'Order has been declined successfully' => 'Dalabaadka si guul leh ayaa la diiday',
    'Hello %name%, Your Demo order for %volume% from %vendor_name% on %date% has been declined. Kindly select another vendor from Demo app' => 'Salaan %name%, dalabaadkaaga Demo ee %volume% ka soo qaaday %vendor_name% %date% waa la diiday. Fadlan xaqiiji dalabaadkaaga biyoolaha dhaamiye app',
